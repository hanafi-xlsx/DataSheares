...
def assignment_charts(array: np.ndarray):
    ...
    xpoints_bar, ypoints_bar, bar_limit = array[:, 0], array[:, 3], [
        800000, 1200000]
    axis[1].set_title(
        "No. of personal vehicles over the years", fontdict=heading_font)
...
"""
desired_charts_menu() lets the user dynamically generate their own desired charts from the data
"""


def desired_charts_menu(array: np.ndarray, types: np.ndarray):
    global type_index
    clear()
    print("You selected 'See your own desired charts'.\n")
    time_input = inquirer.list_input("What kind of data do you want to see?",
                                     choices=[
                                         ("Data across the years", 1), ("Data for a given year", 2), (return_message, 3)],
                                     carousel=True)
    match(time_input):
        # Chosen: data across the years
        case(1):
            data_across_years_menu(array, types)
        # Chosen: data for a given year
        case(2):
            data_given_year_menu(array, types)
        # Chosen: return to view charts menu
        case(3):
            return


def data_given_year_menu(array: np.ndarray, types: np.ndarray):
    clear()
    ...
    chart_input = inquirer.list_input("What kind of chart do you want to see?",
                                      choices=[
                                          ("Bar chart (comparison)", 1), ("Pie chart (composition)", 2), (return_message, 3)],
                                      carousel=True)
    match(chart_input):
        # Chosen: return to view charts menu
        case(3):
        ...
        case(1):
            bar_chart(array, types, year_input)
        # Chosen: Pie chart (composition)
        case(2):
            pie_chart(array, types, year_input)


def pie_chart(array: np.ndarray, types: np.ndarray, year_input: int):
    clear()
...
def bar_chart(array: np.ndarray, types: np.ndarray, year_input: int):
    clear()
...
def data_across_years_menu(array: np.ndarray, types: np.ndarray):
    clear()
    xpoints = array[:, 0]
    print("You selected 'Data across the years'\n")
    variable_input = inquirer.list_input("What kind of data do you want to see?",
                                         choices=[("Univariate (line chart for a single datatype)", 1), (
                                             "Bivariate (single-line chart e.g.: Bus passengers per Bus)", 2), ("Multivariate (multi-line chart)", 3), (return_message, 4)],
                                         carousel=True)
    match(variable_input):
        # Chosen: Univariate (line chart, single type)
        case(1):
            univariate_chart(array, types, xpoints)
        # Chosen: Bivariate (single-line chart e.g.: Bus passengers per Bus)
        case(2):
            bivariate_chart_menu(array, types, xpoints)
        # Chosen: Multivariate (multi-line chart, multiple types)
        case(3):
            multivariate_chart(array, types, xpoints)
        # Chosen: return to view charts menu
        case(4):
            return


def univariate_chart(array: np.ndarray, types: np.ndarray, xpoints: np.ndarray):
    clear()
    ...
    plt.xlabel('Year')
    plt.title(
        f"No. of {types[type_index].lower()} crossing over the years", fontdict=heading_font)
    plt.show()


def bivariate_chart_menu(array: np.ndarray, types: np.ndarray, xpoints: np.ndarray):
    clear()
...
{types[first_type_index]} per ______
    ...
    second_type_index = inquirer.list_input("Select 2nd variable",
                                            choices=[(type, idx) for idx, type in enumerate(
                                                types) if idx != first_type_index],
                                            carousel=True)
    clear()
    print(
        f"You have selected:\n\n{types[first_type_index]} per {types[second_type_index]}.\n")
    sleep(2)
    bivariate_chart(array, types, xpoints, first_type_index, second_type_index)


def bivariate_chart(array: np.ndarray, types: np.ndarray, xpoints: np.ndarray, first_type_index: int, second_type_index: int):
    ...
    plt.xlabel('Year')
    plt.title(
        f"No. of {types[first_type_index]} per {types[second_type_index]} crossing over the years", fontdict=heading_font)
    plt.show()


def multivariate_chart(array: np.ndarray, types: np.ndarray, xpoints: np.ndarray):
    clear()
